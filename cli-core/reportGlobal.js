...
function getWorstEcoIndexes(obj) {
  ...
  obj.pages.forEach(page => {
    worstEcoIndexes = worstEcoIndexes.map((worstEcoIndex, i) => {
      if (page.actions.length === 1 || i === 0) { // first = last if only one value, otherwise return value of first action
        if (page.actions[0].analysis) {
          return getWorstEcoIndex(page.actions[0].ecoIndex, worstEcoIndex);
        }
      } else if (i === 1) { // return value of last action
  ...
  return worstEcoIndexes.map(worstEcoIndex => ({ ecoIndex: worstEcoIndex, grade: getEcoIndexGrade(worstEcoIndex) }));
}
