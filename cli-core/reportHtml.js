...
async function create_html_report(reportObject,options){
    ...
    // copy icons in output folder
    copyIcons(outputFolder);
}
...
function extractBestPractices(bestPracticesFromReport) {
    ...
    bestPracticesKey.forEach(key => {
        const bestPractice = {
            ...
            iconGrade: `icons/${bestPracticesFromReport[key].complianceLevel || 'A'}.png`
        };
...
function copyIcons(outputFolder) {
    const iconsPathFolderDest = path.join(outputFolder, 'icons');
    if (!fs.existsSync(iconsPathFolderDest)){
        fs.mkdirSync(iconsPathFolderDest, { recursive: true });
    }

    const icons = ['A.png', 'B.png', 'C.png'];
    icons.forEach(icon => {
        const iconPathFileDest = path.join(iconsPathFolderDest, icon);
        if(!fs.existsSync(iconPathFileDest)) {
            fs.copyFileSync(`cli-core/template/icons/${icon}`, iconPathFileDest);
        }
    });
}
