...
const create_global_report =
  require('../cli-core/reportGlobal.js').create_global_report;
const create_XLSX_report =
  require('../cli-core/reportExcel.js').create_XLSX_report;
const create_html_report =
  require('../cli-core/reportHtml.js').create_html_report;
...
function getReportFormat(format, filename) {
  ...
  const extensionFormat = formats.find((format) =>
    filenameLC.endsWith(`.${format}`),
  );
  if (extensionFormat) {
    console.log(
      `No output format specified, defaulting to ${extensionFormat} based on output file name.`,
    );
  }
